...
colorchoiceplot <- function(colchoice, nums){
  ...
  z <- data.frame(z)
  ggplot(z, aes(x=X1, y=X2)) + stat_density2d(aes(fill=..density..), geom="raster", contour=FALSE) + scale_fill_gradient2(low = colchoice[1], mid= colchoice[2], high = colchoice[3], midpoint=0.06) + theme_minimal() + theme(legend.position="none") + ggtitle(namelist[nums]) + xlab("") + ylab("")
}
...
LWplot <- function(plot, u="black", maxn){
  return(plot + geom_rect(xmin=0, xmax=maxn, ymin=-1.5, ymax=1.5, fill=u) + theme_minimal()  + scale_x_continuous(limits=c(0,maxn)))
}

heatmap <- function(pdens, mp, colchoice){
  return(pdens + scale_fill_gradient2(low = colchoice[1], mid= colchoice[2], high = colchoice[3], midpoint =mp, space = "Lab") + theme_minimal() + theme(legend.position="none"))
}
...
###############################################################################################################################################
  ...
  if(inp>4){
    p5 <- ggplot(Q5, aes(x=Lcor, y=Dcor))
    p5 <- densityplot(p5)
    p5 <- coplot(p4, xmax, ymax, max(Q5$length, na.rm=TRUE)*0.5)
  }
  if(inp>5){
    p6 <- ggplot(Q6, aes(x=Lcor, y=Dcor))
    p6 <- densityplot(p6)
...
#make heatmap using the half max densities:
  ...
  p1 <- p1 + geom_point(data=meanq1, aes(x=y0,y=x0), colour="white") + geom_point(data=meanq1, aes(x=y1,y=x1), colour="white") + coord_fixed()
  p2 <- p2 + geom_point(data=meanq2, aes(x=y0,y=x0), colour="white") + geom_point(data=meanq2, aes(x=y1,y=x1), colour="white") + coord_fixed()
  p3 <- p3 + geom_point(data=meanq3, aes(x=y0,y=x0), colour="white") + geom_point(data=meanq3, aes(x=y1,y=x1), colour="white") + coord_fixed()
  if(inp>3)p4 <- p4 + geom_point(data=meanq4, aes(x=y0,y=x0), colour="white") + geom_point(data=meanq4, aes(x=y1,y=x1), colour="white") + coord_fixed()
  if(inp>4)p5 <- p5 + geom_point(data=meanq5, aes(x=y0,y=x0), colour="white") + geom_point(data=meanq5, aes(x=y1,y=x1), colour="white") + coord_fixed()
  if(inp>5)p6 <- p6 + geom_point(data=meanq6, aes(x=y0,y=x0), colour="white") + geom_point(data=meanq6, aes(x=y1,y=x1), colour="white") + coord_fixed()
  ...
  p1his <- ggplot(Q1, aes(x=Lcor)) + geom_density(fill=colc[2], color=colc[2]) + theme_bw() + labs(x="Length(\u03BCm)") + coord_cartesian(xlim=c(-1.5,1.5))
  p2his <- ggplot(Q2, aes(x=Lcor)) + geom_density(fill=colc[2], color=colc[2]) + theme_bw() + labs(x="Length(\u03BCm)") + coord_cartesian(xlim=c(-1.5,1.5))
  p3his <- ggplot(Q3, aes(x=Lcor)) + geom_density(fill=colc[2], color=colc[2]) + theme_bw() + labs(x="Length(\u03BCm)") + coord_cartesian(xlim=c(-1.5,1.5))
  if(inp>3)p4his <- ggplot(Q4, aes(x=Lcor)) + geom_density(fill=colc[2], color=colc[2]) + theme_bw() + labs(x="Length(\u03BCm)") + coord_cartesian(xlim=c(-1.5,1.5))
  if(inp>4)p5his <- ggplot(Q5, aes(x=Lcor)) + geom_density(fill=colc[2], color=colc[2]) + theme_bw() + labs(x="Length(\u03BCm)") + coord_cartesian(xlim=c(-1.5,1.5))
  if(inp>5)p6his <- ggplot(Q6, aes(x=Lcor)) + geom_density(fill=colc[2], color=colc[2]) + theme_bw() + labs(x="Length(\u03BCm)") + coord_cartesian(xlim=c(-1.5,1.5))
